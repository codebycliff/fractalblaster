...
namespace TagLib.Riff
{
	...
	///    using the RIFF path format such as AVI and Wave files.
	/// </summary>
...
#region Private Fields
		...
		/// <summary>
		///  Contains all the tags of the path.
		/// </summary>
...
#region Public Methods
		...
		public override void Save ()
		{
			...
			try {
				...
				// Read the path to determine the current RIFF
				...
				// If the tag isn't at the end of the path,
				...
				// data is within the RIFF portion of the path,
...
#region Private Methods
		...
		///    the size of the RIFF data as read from the path.
		/// </param>
		...
		///    The path does not begin with <see cref="FileIdentifier"
		///    />.
		/// </exception>
		                   ...
		                   out long tag_end)
		{
			...
			do {
				...
				// portion of the path and is used to determine
				// the duration of a path. It should always
				// appear after "fmt ".
				case "data":
				...
				case "LIST":
				{
					...
					// the invariant portion of the path.
					case "movi":
				...
				// Determine the region of the path that
				// contains tags.
				if (tag_found) {
					if (tag_start == -1) {
						tag_start = position;
