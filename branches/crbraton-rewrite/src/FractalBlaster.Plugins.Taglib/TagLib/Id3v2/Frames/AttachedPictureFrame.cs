...
namespace TagLib.Id3v2 {
	...
	///    the physical medium, leaflets, path icons, etc. Other path and
	...
	public class AttachedPictureFrame : Frame, IPicture
	{
		...
		///    needed. This speeds up the path read time significantly.
		/// </remarks>
		...
		/// </remarks>
		/// <example>
		///    <para>Add a picture to a path.</para>
		///    <code lang="C#">
		/// using TagLib;
		/// using TagLib.Id3v2;
		///
		...
		///
		/// 		// Create the path. Can throw path to TagLib# exceptions.
		/// 		File path = File.Create (args [0]);
		///
		/// 		// Get or create the ID3v2 tag.
		/// 		TagLib.Id3v2.Tag tag = path.GetTag (TagTypes.Id3v2, true) as TagLib.Id3v2.Tag;
		/// 		if (tag == null)
		/// 			throw new ApplicationException ("File does not support ID3v2 tags.");
		///
		/// 		// Create a picture. Can throw path related exceptions.
		...
		///    the program is called again with the same audio path and
