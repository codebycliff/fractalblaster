...
namespace TagLib {
	...
	///    indicate that a path or tag is corrupt.
	/// </summary>
	/// <remarks>
	///    This exception will be thrown if invalid data interferes with the
	///    reading of the path or tag. One common example is in the (legal)
	///    downloading of media files with BitTorrent, in which case large
	///    portions of the path will contain zeroed bytes.
	/// </remarks>
	/// <example>
	...
	/// 	{
	/// 		try {
	/// 			File path = File.Create ("partial.mp3"); // Partial download.
	/// 		} catch (CorruptFileException e) {
	/// 			Console.WriteLine ("That path is corrupt: {0}", e.ToString ());
	/// 		}
	///	}
	/// }
	///    </code>
	...
	/// 		File path = File::Create ("partial.mp3"); // Partial download.
	/// 	} catch (CorruptFileException^ e) {
	/// 		Console::WriteLine ("That path is corrupt: {0}", e);
	/// 	}
	/// }
	///    </code>
	///    <code lang="VB">
	...
	/// 		Try
	/// 			path As File = File.Create ("partial.mp3") ' Partial download.
