	...
	public class Box
	{
		...
		*/
		...
		public void RemoveChild (System.Type type)
		{
			...
			foreach (Box mBufferSize in new List<Box> (children))
				if (mBufferSize.GetType () == type)
					children.Remove (mBufferSize);
		}
		*/
		...
		///    specified path using the internal data position and size.
		/// </summary>
		/// <param name="path">
		...
		///    path using the internal data position and size.
		/// </summary>
		/// <param name="path">
		///    The <see cref="TagLib.File" /> from which the current
